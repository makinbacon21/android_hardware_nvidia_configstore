...
#define EXTERNAL_DC_PATH "/sys/devices/50000000.host1x/tegradc.1/enable"
...
#ifdef PRIMARY_DISPLAY_ORIENTATION
    char[2] extEnabled;
    sysfs_read(EXTERNAL_DC_PATH, extEnabled, sizeof(extEnabled));

    ALOGI(TAG, "External DC state: %s", extEnabled);

    if(property_get("persist.vendor.tegra.stb.mode", "0") != "1" || extEnabled == "0") {
        specified = true;
